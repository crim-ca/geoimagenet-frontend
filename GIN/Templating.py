...
class Renderer:
    def __init__(self, translations: NullTranslations, default_values: dict = None):
        ...
        protocol = environ.get('SERVER_PROTOCOL') or 'https://'
        default_location = environ.get('SERVER_DOMAIN') or 'geoimagenetdev.crim.ca'
        default_endpoint = f'{protocol}{default_location}'
        self.env.install_gettext_translations(translations, True)
        self.add_global('GEOSERVER_URL', environ.get('GEOSERVER_URL') or f'{default_endpoint}/geoserver')
        self.add_global('GEOIMAGENET_API_URL', environ.get('GEOIMAGENET_API_URL') or f'{default_endpoint}/api/v1')
