...
class Actions extends Component<Props> {
  make_set_mode_callback = (mode: $Values<typeof MODE>) => () => {
    const { store_actions: { set_mode } } = this.props
    set_mode(mode)
  }
  ...
  render() {
    ...
    return (
      <ActionsContainer>
        {
          state_proxy.actions_activated
            ? visible_actions.map((action) => (
              <FontAwesomeIcon
                key={`${action.mode}`}
                icon={action.icon}
                className={action.mode === ui_store.mode_selected ? 'fa-2x active' : 'fa-2x'}
            ...
            : visible_actions.map((action) => (
              <FontAwesomeIcon
                key={`${action.mode}`}
                icon={action.icon}
                className={action.mode === state_proxy.mode ? 'fa-2x active' : 'fa-2x inactive'}
              />
            ))
        }
      </ActionsContainer>
