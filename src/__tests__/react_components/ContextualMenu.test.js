...
describe('We should be able to instantiate the contextual menu container and use it to display options.', () => {
  ...
  test('Creating the container registers the populate menu callback', () => {
    ...
    expect(ContextualMenuManager.populate_menu_callback)
      .toBe(null);
  ...
  test('Calling the choose_option manager\'s method renders a dialog', async () => {
    ...
    expect(wrapper.state().menu_items[0].text)
      .toEqual('option 1 text');

    wrapper.unmount();
  });

  test('Calling the confirm method without container correctly throws', async () => {
    await expect(ContextualMenuManager.choose_option(default_options))
      .rejects
      .toEqual('There is no populate menu callback registered. Did you instantiate a ContextualMenuContainer?');
    ...
    await expect(ContextualMenuManager.choose_option(default_options))
      .rejects
      .toEqual('The populate menu callback registered is not a function. ' +
  ...
  test('We enforce to have more than a single option when choosing options', async () => {
    ...
    await expect(ContextualMenuManager.choose_option([
      ...
      .rejects
      .toEqual('You should ask for at least two options for the user to choose from.');
    wrapper.unmount();
  });
